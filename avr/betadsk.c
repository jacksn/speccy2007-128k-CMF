...
#define RDCRC_TYPEI        0
	...
	#define strcasecmp_P stricmp
	#define pgm_read_byte(x) *(byte *)(x)
	unsigned char trdos_seek;
	unsigned char trdos_curcyl;
	#define DRAW_SEEK
...
struct sys_sec_area PROGMEM scl_sys_sec_def =
{
...
struct img_fmt_def PROGMEM img_fmt_def[NUM_DSK_FORMATS] =
{
	{ DISK_TRD, { "trd" } },
	{ DISK_SCL | 0x80, { "scl" } },
	{ DISK_FDI, { "fdi" } }
...
#if defined(WIN32_TEST) && BETADSK_TRACE
void __TRACE( const char *fmt, ... )
{
	va_list args;
	va_start( args, fmt );
	vfprintf( f_portdbg, fmt, args );
	fflush( f_portdbg );
	va_end( args );
}
#endif
...
//  strcpy(ext_buf, p_ext);
//  strlwr(ext_buf);

	for ( fmt = DISK_NONE, i = 0; i < NUM_DSK_FORMATS; i++ )
	{
		if ( strcasecmp_P( p_ext + 1, img_fmt_def[i].ext ) == 0 )
		{
			fmt = pgm_read_byte( &img_fmt_def[i].fmt );
			break;
		}
	}
	...
	if ( fmt == DISK_SCL )
	{
		...
		sys_area = ( struct sys_sec_area * )drives[drv_id].scl_sys_sec;
		memcpy_P( sys_area, &scl_sys_sec_def, 32/*sizeof(sys_area)*/ );
		for ( ; ( char )i >= 0; i-- )
		{
			sys_area->label[i] = toupper( filename[i] );
		}
		for ( i = 0; i < 8; i++ )
		{
...
zero_blk:
			...
			if ( cur_drv->sec_off > ((( dword )cur_drv->scl_last_sec ) * (( dword )cur_drv->sec_size ) + (( dword )cur_drv->scl_data ) - cur_drv->img_trkoff ) )
			{
				goto zero_blk;
			}
		}
...
#if BETADSK_TRACE
	#if defined(WIN32_TEST)
		f_portdbg = fopen( "bdi.log", "wt" );
	#else
		f_open( &f_trace, "bdi.log", FA_WRITE | FA_CREATE_ALWAYS );
	#endif
#endif
